...
#### GRADIENTS ###
	...
	mix_color_edges.location = (-920, -200)
...
#### IMAGE TEXTURE ###
	...
	mix_color_text.location = (-700, -80)
...
#### FALLOFF https://blenderartists.org/t/light-falloff-light-decay/1196885/14 ###

	# Geometry
	falloff_geometry = mat.node_tree.nodes.new('ShaderNodeNewGeometry')
	falloff_geometry.name = "Falloff_Geometry"
	falloff_geometry.location = (-1160.0,-660.0)

	# Dot product
	falloff_dot = mat.node_tree.nodes.new('ShaderNodeVectorMath')
	falloff_dot.name = "Falloff_Dot"
	falloff_dot.operation = 'DOT_PRODUCT'
	mat.node_tree.links.new(falloff_geometry.outputs[3], falloff_dot.inputs[0])
	mat.node_tree.links.new(falloff_geometry.outputs[4], falloff_dot.inputs[1])
	falloff_dot.location = (-980.0,-680.0)

	# Subtract
	falloff_subtract = mat.node_tree.nodes.new('ShaderNodeMath')
	falloff_subtract.operation = 'SUBTRACT'
	falloff_subtract.name = "Falloff_Subtract"
	falloff_subtract.use_clamp = True
	mat.node_tree.links.new(falloff_dot.outputs["Value"], falloff_subtract.inputs[0])
	falloff_subtract.location = (-800.0,-680.0)

	# Multiply
	falloff_multiply = mat.node_tree.nodes.new('ShaderNodeMath')
	falloff_multiply.operation = 'MULTIPLY'
	falloff_multiply.name = "Falloff_Multiply"
	mat.node_tree.links.new(falloff.outputs[0], falloff_multiply.inputs[0])
	mat.node_tree.links.new(falloff_subtract.outputs[0], falloff_multiply.inputs[1])
	falloff_multiply.location = (-620.0,-500.0)
...
def update_mat(self, context):
	...
	try:
		...
		if light.type == "MESH":
			...
			falloff_multiply = mat.node_tree.nodes["Falloff_Multiply"]
			falloff_subtract = mat.node_tree.nodes["Falloff_Subtract"]
			ies = mat.node_tree.nodes["IES Texture"]
			...
			falloff_subtract.inputs[1].default_value = light.Lumiere.falloff_strength
			...
			if light.Lumiere.falloff_type == '0':
				# Quadratic
				if light.Lumiere.falloff_strength > 0:
					mat.node_tree.links.new(falloff.outputs[0], falloff_multiply.inputs[0])
		...
		print("\n[Lumiere ERROR]\n")
		print("\nError : Updating material to try to resolve. (Check the console for more info)\n")
		import traceback
		traceback.print_exc()
		softbox_mat(light)
...
#### FALLOFF https://blenderartists.org/t/light-falloff-light-decay/1196885/14 ###
	# Geometry
	falloff_geometry =light.data.node_tree.nodes.new('ShaderNodeNewGeometry')
	falloff_geometry.name = "Falloff_Geometry"
	falloff_geometry.location = (-460, -160)

	# Dot product
	falloff_dot = light.data.node_tree.nodes.new('ShaderNodeVectorMath')
	falloff_dot.name = "Falloff_Dot"
	falloff_dot.operation = 'DOT_PRODUCT'
	light.data.node_tree.links.new(falloff_geometry.outputs[3], falloff_dot.inputs[0])
	light.data.node_tree.links.new(falloff_geometry.outputs[4], falloff_dot.inputs[1])
	falloff_dot.location = (-280, -160)

	# Subtract
	falloff_subtract = light.data.node_tree.nodes.new('ShaderNodeMath')
	falloff_subtract.operation = 'SUBTRACT'
	falloff_subtract.name = "Falloff_Subtract"
	falloff_subtract.use_clamp = True
	light.data.node_tree.links.new(falloff_dot.outputs["Value"], falloff_subtract.inputs[0])
	falloff_subtract.location = (-100, -100)

	# Multiply
	falloff_multiply = light.data.node_tree.nodes.new('ShaderNodeMath')
	falloff_multiply.operation = 'MULTIPLY'
	falloff_multiply.name = "Falloff_Multiply"
	light.data.node_tree.links.new(falloff.outputs[0], falloff_multiply.inputs[0])
	light.data.node_tree.links.new(falloff_subtract.outputs[0], falloff_multiply.inputs[1])
...
def update_lamp(light):
	...
	try:
		...
		rgb.outputs[0].default_value = light.Lumiere.light_color
		blackbody_color.inputs[0].default_value = light.Lumiere.blackbody
		ies.inputs[1].default_value = 1 #light.Lumiere.energy
		...
		if light.Lumiere.light_type in ("Spot", "Point", "Area"):
			...
			if light.Lumiere.falloff_type == '0':
				...
				if light.Lumiere.falloff_strength > 0:
					mat.node_tree.links.new(falloff.outputs[0], falloff_multiply.inputs[0])
					mat.node_tree.links.new(falloff_multiply.outputs[0], emit.inputs[1])
				else:
